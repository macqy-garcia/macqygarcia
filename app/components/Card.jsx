    ...
    <div className="flex w-full flex-col gap-7 bg-[#0C1F34] px-7 py-8 shadow-sm">
      <div>
        ...
        <h3 className="text-xl font-semibold tracking-wide text-[#E6F1FF] hover:text-[#64ffda]">
          <NavLink to="/">{project.title}</NavLink>
        </h3>
      </div>
      <p className="text-sm text-[#a8b2d1] lg:max-w-lg">{project.overview}</p>
      <p className="font-mono text-xs text-[#a8b2d1]">{project.stack}</p>
...
export const SmallCard = ({ project }) => {
  return (
    <div className="relative flex cursor-pointer flex-col gap-7 bg-[#0C1F34] px-5 py-8 shadow-sm transition delay-150 duration-300 ease-in-out hover:-translate-y-2">
      ...
      <p className="mb-10 text-sm text-[#a8b2d1]">{project.overview}</p>
      <p className="absolute bottom-5 left-5 font-mono text-xs text-[#a8b2d1]">
        {project.stack}
      </p>
    </div>
  );
